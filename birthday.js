...
/* ============================================================
   ...
   ============================================================ */
const PHOTOS = [
  { src: '/photos/memory-hero.jpg', caption: 'mera babu 🧿🧿❤️' },
  { src: '/photos/memory-01.jpg', caption: 'a little moment I want to keep forever ❤️' },
];
...
// Sizes the frame to the photo's real pixel aspect ratio, so the border
// always hugs the picture exactly instead of leaving empty space on the
// sides. Computed in JS (not CSS aspect-ratio) because the slide's <img>
// is position:absolute and contributes no intrinsic size to the stage,
// which otherwise collapses to 0x0.
...
function setGiftRotation(deg){
  giftRotation = deg;
  giftBox.style.transform = `translate(-50%,-50%) rotateX(-4deg) rotateY(${deg}deg)`;
}
...
function revealLetter(){
  ...
  if (paper) paper.classList.add('is-writing');
  ...
  const typeLine = () => {
    ...
    const raw = line.getAttribute('data-letter-text') || '';
    line.classList.add('is-writing','is-current');
    let char = 0;
    const tick = () => {
      if (char <= raw.length){
        line.innerHTML = raw.slice(0, char)
          .replace(/&/g,'&amp;')
          .replace(/</g,'&lt;')
          .replace(/>/g,'&gt;')
          .replace(/"/g,'&quot;')
          .replace(/\n/g,'<br>');
        char++;
        letterTypingTimer = setTimeout(tick, 18);
      } else {
        line.classList.remove('is-current');
        lineIndex++;
        letterTypingTimer = setTimeout(typeLine, 260);
      }
    };
    tick();
  };
